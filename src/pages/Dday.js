...
function Dday(){
    ...
    return(
        ...
        {
            !Dday ? (
                <p>자신만의 디데이를 설정해주세요.<br/>
                <span onClick={handleOpen}>디데이 설정하러 가기</span></p>
            ):(
                <>
                <p>{title}</p>
                <p>D - {Dday} 일 남았습니다!</p>
                <span onClick={handleOpen}>다른 D-day 설정하기</span>
                </>
            )
        }
        ...
        >
            <Box sx  ={style}>
                <Typography id="modal-modal-description" sx ={{mt:2}}>
                    ...
                    <input style={{border:"1px solid black"}} onChange={inputtitle} placeholder="일정 이름" value={title} type="text"/>
