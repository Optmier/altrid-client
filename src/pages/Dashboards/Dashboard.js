...
`;
const GreetingsLeftInfo = styled.div``;
const GreetingsLeftInfoTop = styled.div`
    align-items: center;
    display: flex;
    position: relative;
`;
...
`;
const GreetingsLeftInfoBottom = styled.div`
    margin-top: 16px;
    @media (max-width: 640px) {
        margin-top: 8px;
        margin-bottom: 8px;
...
`;
const DDayContainerMobile = styled.div`
    display: none;
    position: absolute;
    @media all and (max-width: 640px) {
        display: initial;
...
`;
const GreetingsRightIls = styled.div`
    background-image: url('/bg_images/dashboard_page/dashboard_icon.png');
...
`;
const TodayWordCard = styled.div`
...
function Dashboard({ match, history }) {
    ...
    const todayWordCardRef = useRef();
    const todayWordRef = useRef();
    const todayWordSizeScaler = (windowWidth) => {
        if (!todayWordRef.current || !todayWordRef.current) return;
        const wordCardPadding = windowWidth > 640 ? 32 : 24;
        const wordCardInnerWidth = todayWordCardRef.current.clientWidth - wordCardPadding * 2;
        const wordCardTextWidth = todayWordRef.current.clientWidth;
        const scaler = wordCardInnerWidth / wordCardTextWidth > 1 ? 1 : wordCardInnerWidth / wordCardTextWidth;
        todayWordRef.current.style.transform = `scale(${scaler})`;
    };
    ...
    useEffect(() => {
        ...
        if (!word) return;
        todayWordSizeScaler(screenWidth);
    }, [screenWidth, word, korean]);

    const cardSelectRender = (layoutSet) => {
        const todayWordCard = (
            <GridResponsive item xs={12} xl={4} sm={layoutSet ? 6 : 4}>
                <DashCard color="orange" ref={todayWordCardRef}>
                    <DashCardHeader>
                        <DashCardTitle>
                    ...
                    <DashCardContents color="orange" disabled={!word}>
                        {!word ? (
                            <Typography type="label" size="xl" bold>
                                단어가 없습니다
                            </Typography>
                        ) : (
                            <TodayWordCard className="today-words-card" onClick={ClickCard}>
                                <span ref={todayWordRef}>{flip ? korean : word}</span>
                            </TodayWordCard>
                        )}
                    </DashCardContents>
            ...
            <Container>
                <Greetings>
                    <GreetingsLeftInfo>
                        ...
                        <GreetingsLeftInfoBottom>
                            <Typography type="heading" size="l" bold>
                                {classInfo.name} 클래스 입니다.
                            </Typography>
                        </GreetingsLeftInfoBottom>
                        <DDayContainerMobile>
                            <Dday classNum={num} />
                    ...
                    <GreetingsRightIls></GreetingsRightIls>
                </Greetings>
                <Box sx={{ flexGrow: 1 }}>
                    <GridResponsive container spacing={4}>
                            ...
                            <DashCard color="yellow">
                                ...
                                <DashCardContents color="yellow" disabled={!total.total}>
                                    {!total.total ? (
                                        ...
                                        <>
                                            <ReactApexChart options={chart.options} series={chart.series} type="radialBar" height={240} />
                                        </>
                                    )}
                                </DashCardContents>
                                <DashCardBottom className="center">
                        ...
                        <GridResponsive item xl={8} sm={layoutSet ? 6 : 8} xs={12}>
                        ...
                        <GridResponsive item xl={4} sm={layoutSet ? 6 : 4} xs={12}>
